...
contract InferenceBilling {
     ...
     * @return _jobId Unique identifier for this inference job
     */
    function payForInference(
        string memory _modelId,
        bytes32 _promptHash
    ) external payable returns (bytes32 _jobId) {
        ModelEndpoint storage model = models[_modelId];
        ...
        emit InferenceRequested(_jobId, _modelId, msg.sender, msg.value);
    }
    ...
    function settleJob(bytes32 _jobId) external {
        ...
        model.totalEarnings += job.amount;
                ...
                emit JobSettled(_jobId, providerAmount, curatorAmount, protocolAmount);
    }
